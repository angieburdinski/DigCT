...
sw = pickle.load(gzip.open('/Users/angeliqueburdinski/expotracing/_qsuite/tracing_sim/results_sw_noQ_NMEAS_100_ONLYSAVETIME_False/results.p.gz','rb'))
sw = np.array(sw)
exp = pickle.load(gzip.open('/Users/angeliqueburdinski/expotracing/_qsuite/tracing_sim/results_exp_noQ_NMEAS_100_ONLYSAVETIME_False/results.p.gz','rb'))
exp = np.array(exp)
marker = ['o','v','x','d','p','<']
colors = [
        'k',
...
def relative_outbreak_reduction():
    ...
    fig.tight_layout()
    plt.savefig('rel_red_RX_noQ',dpi = 300)
    plt.show()
...
def darkfactor():
    ...
    fig.tight_layout()
    #plt.savefig('darkfactor_noQ',dpi = 300)
    plt.show()
