...
def swnetwork(N, parameter,**kwargs):
    ...
    beta = 10e-7 #for k = 20, N = 200_000 or k=10
    #beta = 0.0001 # for N = 2000 k = 20 or k = 20*0.6
    #beta = 0.00001 # for N = 5000 k = 20 or k = 20*0.6
    G = get_smallworld_graph(N,k_over_2,beta)
    edge_weight_tuples = [ (e[0], e[1], 1.0) for e in G.edges() ]
    k_norm = 2*len(edge_weight_tuples) / N
    del G
    return edge_weight_tuples, k_norm
def ernetwork(N, parameter,**kwargs):
    p = parameter
    k_over_2 = int(p['number_of_contacts']/2)
...
def simulation_code(kwargs):
    ...
    def mixed(networks, q, a, lockdown, y, N, quarantiningS, parameter, sampling_dt, time, clustered, **kwargs):
        p = parameter.copy()
        if lockdown == True:
            p['number_of_contacts'] = parameter['number_of_contacts']*0.6
            p['R0'] = parameter['R0']*0.6

        if networks == 'ER':
            edge_weight_tuples, k_norm = ernetwork(N,p)

        if networks == 'SW':
            edge_weight_tuples, k_norm = swnetwork(N,p)
        if networks == 'ER_exp':
            edge_weight_tuples, k_norm = confignetwork(N,p)

        if networks == 'SW_exp':
            edge_weight_tuples, k_norm = exp_sw_network(N,p)
        ...
        if clustered == True:
            ...
            if 0 < a < 1:
                app_user = get_app_user(N, app = a)
                not_app_user = [i for i in list(range(N)) if i not in app_user]

                I_Pa_list = list(np.random.choice(app_user, int(a*p['I_0']), replace=False))
