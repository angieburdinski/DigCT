...
def simulation_code(kwargs):
    ...
    def mixed(N, parameter, time, sampling_dt,quarantiningS, a, q, y, clustered,**kwargs):
        p = parameter
        ...
        if clustered == True:
            ...
            if 0 < a < 1:
                ...
                I_Pa_list = list(np.random.choice(app_user, int(p['I_0']), replace=False))
                Sa_list = [i for i in app_user if i not in I_Pa_list]
                I_P_list = list(np.random.choice(not_app_user,  int(p['I_0']), replace=False))
        ...
        results = I_Pa_list+Sa_list,max(result['R']),max(result['Ra']),max(result['X']),max(result['Xa']),max(result['C'])
        del result
        return results
